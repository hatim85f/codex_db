  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!isMatch) {
        return res
          .status(400)
          .json({ message: "Invalid Username or Password", error: "Error!" });
      }
...
router.post("/request-reset", async (req, res) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ error: "Error!", message: "Email is required" });
  }
...
router.put("/change-password/:id", auth, async (req, res) => {
  ...
  if (!currentPassword || !newPassword) {
    return res
      .status(400)
      .json({ error: "Error!", message: "All fields are required" });
  }
  if (!mongoose.isValidObjectId(id)) {
    return res
      .status(400)
      .json({ error: "Error!", message: "Invalid user id" });
  }
  ...
  try {
    ...
    if (isSelf || !isAdminLike) {
      ...
      if (!ok)
        return res
          .status(400)
          .json({ error: "Error!", message: "Old password is incorrect" });
    }
